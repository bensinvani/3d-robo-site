...
export function Hero() {
  ...
  const onProgress = (p: number) => {
    if (hint.current) {
      hint.current.style.opacity = String(Math.max(0, 1 - p / 0.05));
    }
    if (copy.current) {
      const t = Math.min(1, Math.max(0, (p - 0.55) / 0.3));
      copy.current.style.opacity = String(1 - t);
      copy.current.style.transform = `translateY(${-40 * t}vh)`;
    }
  };
  ...
  return (
    ...
    >
      ...
      <div ref={hint} className="absolute bottom-6 left-1/2 -translate-x-1/2 font-body text-[10px] tracking-[0.4em] text-mist/50 uppercase">
        scroll
      </div>
    </SequenceCanvas>
  );
